...
interface EditDiscountModalProps {
  ...
  onUpdate?: (discountId: string, discount: {
    name: string;
    value: number;
    type: "%" | "$";
    isDefault: boolean;
...
const EditDiscountModal = ({
  ...
  // Pre-fill form when modal opens or discount changes
  useEffect(() => {
    if (open && discount) {
      setDiscountName(discount.name);
      setDiscountValue(discount.value.toString());
      setDiscountType(discount.type);
      // Note: The mock data doesn't have an isDefault field, so we'll default to false
      // In a real app, this would come from the discount object
      setIsDefault(false);
    }
  }, [open, discount]);
  ...
  const handleUpdate = () => {
    ...
    if (onUpdate) {
      ...
      console.info("Updating discount", {
        id: discount.id,
        name: discountName.trim(),
        value: value,
        type: discountType,
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="w-[90%] max-w-sm mx-auto p-5 rounded-2xl shadow-lg bg-white [&>button]:hidden">
        ...
        <DialogHeader className="flex flex-row items-center justify-between pb-2 border-b border-gray-100">
          <DialogTitle className="text-base font-semibold text-gray-800">Edit Discount</DialogTitle>
          <Button
            variant="ghost"
            size="icon"
